...
- **Sessions are database-backed, so there is no signing secret.** `POST /api/login` verifies the PIN and inserts a `sessions` row; the cookie carries a random 256-bit token and only the token's SHA-256 hash is stored, so a database dump can't be replayed as a live session. `DATABASE_URL` is the only required env var. Don't reintroduce `SESSION_SECRET` — it was removed deliberately so deploys need no extra configuration.
- **Two layers, on purpose.** `proxy.ts` (this version's `middleware.ts` — renamed in Next 16) only checks that a session cookie is *present*; it cannot validate it, because that needs a DB read and the proxy runs on every request including prefetches. Every page and mutating Server Action calls `requireSession()` for the real check. Server Actions are directly POST-reachable, so the proxy alone would not protect them — if you add an action or page, add the `requireSession()` call too.
- **The proxy redirect is one-directional on purpose.** It never bounces a cookie-bearing user away from `/login`; a stale cookie would then ping-pong between `/` and `/login` forever. `app/login/page.tsx` does that redirect after a real session check instead.
- Failed logins return one generic "Incorrect PIN." for both a malformed PIN and a wrong one, so the response reveals nothing.
- Logout deletes the `sessions` row, not just the cookie — otherwise a captured token would stay usable.
